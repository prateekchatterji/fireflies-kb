...
- **Idempotency**: `fetch_meetings.py` skips meetings already on disk. If a meeting's transcript was updated *after* you first fetched it (rare), your local copy is stale. To force a refresh of one meeting: `rm data/raw/<id>.json` and re-run.
...
2. **Detail query** — one per meeting *that isn't already on disk*. Deduplicated by the `if out_path.exists(): skipped += 1; continue` check.
...
A typical flow: write a small Python script that loops over `data/bundles/*.md`, sends each to Ollama with a prompt like *"extract action items assigned to Avni"*, and writes results to `data/digests/`.
...
Since you mentioned you can run Claude Code: it's overkill for routine extraction over bundles (and the cost adds up), but it's useful for *one-off* heavier asks — e.g. "read all bundles from the last month and draft a board update." Use it sparingly for high-leverage work, not for routine digesting.
...
Long-form multi-line commits are welcome when the *why* isn't obvious from the diff.
